...
// 친구 목록 조회 권한 동의
// 테스트할 계정만 동의가 된 것.
// 비즈니스앱이 되어야 모든 친구의 목록을 조회할 수 있다.
// 지금은 테스트 계정의 친구목록만(개발자 페이지에)
app.get("/friend", (req,res)=> {
    const friend_url = `https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${process.env.KAKAO_CLIENT_ID}&redirect_uri=${process.env.REDIRECT_URL}&scope=friends`
    res.redirect(friend_url);
})

// 친구 목록 확인
// 친구를 조회했을때 내가 만든 서비스에서 그 친구의 고유 아이디는 uuid이다. uuid를 알고 있어야 카카오톡 메세지 혹은 소셜 요청을 보낼 수 있다.
app.get("/frendsView", async(req,res)=>{
    const {kakao_access_token} =req.cookies
    console.log(kakao_access_token);
    const friends_view_url = "https://kapi.kakao.com/v1/api/talk/friends"
    const data = await axios.get(friends_view_url,{
        headers : {
...
app.get("/myMessage", async(req,res)=> {
    ...
    // 내장객체에서 쿼리스트링을 만들때 쉽게 만들수 있도록 제공하는 내장 모듈
    // 쿼리스트링으로 변환 해주는 내장 모듈. 객체를 전달하면 변환해준다.
    //const params = qs.stringify({a : {b : {c : 1}}}) //> a%5Bb%5D%5Bc%5D=1
...
app.post("/friendMessage", async(req,res)=> {
    const url = `https://kapi.kakao.com/v1/api/talk/friends/message/send`
